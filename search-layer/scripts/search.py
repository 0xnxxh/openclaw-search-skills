...
def _first_author_token(authors: list[str]) -> str:
    if not authors:
        return "Unknown"
    first = re.sub(r"[^a-zA-Z0-9]", "", authors[0].split()[-1])
...
def build_export_record(result: dict) -> dict:
    doi = normalize_doi(result.get("doi"))
    authors = normalize_authors(result.get("authors"))
    if not authors:
        snippet = result.get("snippet", "")
        fallback = snippet.split(".")[0] if snippet else ""
        authors = normalize_authors(fallback)

    venue = (result.get("venue") or "").strip()
    if not venue:
        match = re.search(r"\[([^\]]+)\]", str(result.get("snippet", "")))
        if match:
            venue = match.group(1).strip()
    ...
    paper_id_type, paper_id = build_paper_identity({
        **result,
        "doi": doi,
    })
    return {
        ...
        "url": result.get("url", ""),
        "source": source,
        "area": result.get("area", "general_cs"),
        "paper_id": result.get("paper_id", paper_id),
        "paper_id_type": result.get("paper_id_type", paper_id_type),
    }
...
def export_bibtex(results: list, query: str) -> str:
    entries = []
    for r in results:
        row = build_export_record(r)
        title = row["title"].replace("{", "\\{").replace("}", "\\}")
        authors = row["authors_text"].replace("{", "\\{").replace("}", "\\}")
        cite_key = f"{_first_author_token(row['authors'])}{row['year']}{_first_title_token(row['title'])}"
        cite_key = re.sub(r"[^a-zA-Z0-9]", "", cite_key) or "Paper"
        lines = [
            f"@article{{{cite_key},",
            f"  title = {{{title}}},",
            f"  author = {{{authors}}},",
...
def export_csv(results: list) -> str:
    ...
    writer.writerow(["title", "authors", "year", "venue", "citation_count", "doi", "url", "source", "area", "paper_id"])
    for r in results:
        row = build_export_record(r)
        writer.writerow([
            row["title"],
...
def export_markdown(results: list, query: str) -> str:
    ...
    for i, r in enumerate(results, 1):
        ...
        lines.extend([
            f"## {i}. {row['title']}",
            "",
            f"- **作者**: {row['authors_text'] or 'N/A'}",
            f"- **年份**: {row['year'] or 'N/A'}",
            f"- **Venue**: {row['venue'] or 'N/A'}",
            f"- **引用**: {row['citation_count']}",
            f"- **DOI**: {row['doi'] or 'N/A'}",
            f"- **来源**: {row['source']}",
            f"- **领域**: {row['area']}",
            f"- **链接**: [{row['url']}]({row['url']})",
            "",
        ])
    return "\n".join(lines)
...
def export_citations(results: list) -> str:
    ...
    for r in results:
        ...
        doi_or_url = f"https://doi.org/{row['doi']}" if row["doi"] else row["url"]
        lines.append(f"{row['authors_text']}. ({row['year']}). {row['title']}.{venue} {doi_or_url}")
    return "\n".join(lines)
